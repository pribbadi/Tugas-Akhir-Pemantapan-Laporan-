<?php 
	...
	if(isset($_POST['username']) AND ($_POST['password'])){ //isset= mengecek apakah ada data yang di masukkan dari inputan username dan password
		...
		$result = mysqli_query($link,"select * from admin where id_admin = '$username' AND password = '$password' AND status='1'");//membuatvariabel result untuk melihat semua data dari tabel admin
		...
		if ($jumlahditemukan == 1){ //kondisi :melihat data per rows dari tabel admin apakah ada(1) atau tidak
		  	...
		  	header("location:../mhs/");//  meredirect ke kehalaman admin/index.php  
	 	}
